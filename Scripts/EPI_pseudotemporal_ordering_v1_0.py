...
def PTO_draw_MST_groups(MST, MST_pos, diam_edges, sample_groups, cmap = plt.cm.jet, node_size = 100, 
    ...
    x_pos = [pos[0] for pos in list(MST_pos.values())]
    y_pos = [pos[1] for pos in list(MST_pos.values())]
    ...
    for node in diam_path:
        ...
        for subnode in MST.neighbors(node):
            if subnode not in diam_path:
                ...
                branches[node].append(list(nx.single_source_dijkstra_path(MST_copy, subnode).keys()))
                
    #2. Find subgraphs
                
    subgraphs = {}
    for root in list(branches.keys()):
        subgraphs[root] = []
...
def PTO_distance_matrix(dataset, node_dict):
    ...
    dist_int = pd.DataFrame(columns = list(node_dict.values()), index = list(node_dict.values()))
...
def PTO_ordered_path(PTO, node_dict):
    ...
    node_dict_inv = {v:k for k, v in list(node_dict.items())}
...
def PTO_robustness_tsne(dataset, iterations, dview, modus, resampling=0.2, return_min = 50,
    ...
    output = pd.DataFrame(index = list(range(iterations)), columns = dataset.columns)
...
def PTO_correlation_robustness_v1P(dataset, PTO_fitted, cells, genes, dview, iterations, resampling=0.5):
    ...
    output = pd.DataFrame(index = list(range(iterations)), columns = cells)
