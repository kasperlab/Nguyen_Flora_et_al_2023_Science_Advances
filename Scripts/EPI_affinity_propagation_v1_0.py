...
from WND_misc_scripts_v1_0 import *

################################################################################
################################# CLUSTERING ###################################
################################################################################

def AP_clustering_v3P(dataset, aff_matrix, axis, affinity, preference, damping, path, max_iter=200, get_parameters=False,
                     convergence_iter=15, copy=True, verbose=False, nearest_neighbour = False, linkage='ward' ):
    ...
    get_parameters: 
        False - returns only sorted af labels
        True - returns a tuple of sorted af labels and used parameters of preference and damping
        Added to accomodate error handling during initial clustering when number of clusters with given preference and damping
        is too low to perform Ward's linkage. 
    ...
    * and additional function arguments are specified in 
    ...
    Z_cluster = sch.linkage(cluster_dist, method=linkage)
    ...
    if get_parameters:
        return (labels[af_sorted], preference, damping)
    else:
...
def AP_IC_v3P(dataset, aff_matrix, axis, affinity, preference, damping, path, dview, linkage='ward',
    ...
    pref, damp = list(zip(*[x for x in itertools.product(preference, damping)]))

    l_map = len(pref)
    
    #do AP clustering in parallel
    """ap, pref_ap, damp_ap = map(AP_clustering_v3P, [dataset] * l_map, [aff_matrix] * l_map, [axis] * l_map,[affinity] * l_map, pref, damp,[path] * l_map)"""
    
    #Couldn't get parallel working, instead, use a for loop
    #Error handling inserted because low cluster numbers can't be used with Ward's linkage - gives error
    #Funcion returns a tuple with (ap, preference, dampening) that are appended to their respective lists for later DataFrame construction (so that lists corresbond to each other)
    ap=[]
    pref_ap=[]
    damp_ap=[]
    for pos, num in enumerate(pref):
        try:
            tmp = AP_clustering_v3P(dataset, aff_matrix, axis, affinity, num, damp[pos], path, linkage=linkage, get_parameters=True)
            ap.append(tmp[0])
            pref_ap.append(tmp[1])
            damp_ap.append(tmp[2])
        except ValueError:
            continue
    
    #calculate IC value
    """ic = dview.map_async(calculateIC_v1P,
                        [dataset] * l_map, 
                        ap,
                        [axis] * l_map,
                        [criterion] * l_map)"""
    ...
    ic=[]
    for item in ap:
        ic_tmp = calculateIC_v1P(dataset, item, axis, criterion)
        ic.append(ic_tmp)
    
    #update output DataFrames
    for P, D, A, I in zip(pref_ap, damp_ap, ap, ic):
        IC.ix[D, P] = I
        Ng.ix[D, P] = len(set(A))
    return IC, Ng
...
def AP_IC_findmin(IC):
    ...
    if len(preference) > 1:
        ...
        preference = preference[int(len(preference) / 2 + len(preference) % 2) - 1]
    ...
    if len(damping) > 1:
        ...
        damping = damping[int(len(damping) / 2 + len(damping) % 2) - 1]
    ...
    return round(preference,3), round(damping,3)
...
def robustness_AP_v1P(dataset, groups, genes, resampling, iterations, preference, damping, dview):
    ...
    def calculate_robustness(dataset, aff_matrix, groups, resampling, iterations, preference, damping):
        ...
        for gr in set(groups):
            ...
            robustness_tmp.ix[gr] = np.max(list(Counter(groups_tmp[ix_tmp]).values())) / float(len(ix_tmp))
        ...
        for gr in set(groups_shuffle):
            ...
            robustness_null_tmp.ix[gr] = np.max(list(Counter(groups_tmp[ix_tmp]).values())) / float(len(ix_tmp))
    ...
    robustness = pd.DataFrame(columns = [str(x) for x in set(groups)], index = list(range(iterations)))

    robustness_null = pd.DataFrame(columns = [str(x) for x in set(groups)], index = list(range(iterations)))
