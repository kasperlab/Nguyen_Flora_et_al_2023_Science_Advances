...
def ImportandMerge(master_file):
    ...
    for barcode in master_file:
        ...
        meta_gen_tmp = meta_gen_tmp.reindex(columns = list(range(1, len(seq_tmp.columns) + 1)))
        ...
        meta_mouse_tmp = meta_mouse_tmp.reindex(columns = list(range(1, len(seq_tmp.columns) + 1)))
        ...
        spikes = open('C:\\Users\\karlan\\Google Drive\\MKA\\Epidermal stem cells at the point of no return\\Sequencing results\\spikes.txt', 'r').read().split('\n')[:-1]
        repeats = open('C:\\Users\\karlan\\Google Drive\\MKA\\Epidermal stem cells at the point of no return\\Sequencing results\\repeats.txt','r').read().split('\n')[:-1]
